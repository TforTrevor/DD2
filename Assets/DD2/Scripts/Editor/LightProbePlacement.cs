...
public class LightProbePlacement : EditorWindow {
    ...
    float distanceAboveGround = 0.5f;
    int layers = 1;
    float layerHeight = 1;
			...
			if(p != null) {
				...
				foreach(ProbeGenPoint pro in probeGen) {
					if(pro.used == false) {
						...
						newProbe += Vector3.up * distanceAboveGround;
	...
	void OnGUI() {
        ...
        distanceAboveGround = EditorGUILayout.FloatField("Distance above ground", distanceAboveGround);
        layers = EditorGUILayout.IntField("Number of layers", layers);
